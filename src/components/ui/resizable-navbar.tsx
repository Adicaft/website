...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "absolute inset-0 hidden flex-1 flex-row items-center justify-center space-x-2 text-sm font-medium text-slate-300 transition duration-200 hover:text-white lg:flex lg:space-x-2",
        className,
      )}
    >
      {items.map((item, idx) => (
        ...
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-xl bg-slate-700/50"
            />
          )}
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className={cn(
            "w-full flex flex-col items-start justify-start gap-4 rounded-lg bg-slate-800/95 px-4 py-6 mt-4 border border-slate-700/50",
            className,
          )}
        >
          {children}
...
export const NavbarLogo = () => {
  return (
    ...
    >
      ...
      <span className="font-bold text-white text-xl">Adityakeyedits</span>
    </a>
  );
};
...
export const NavbarButton = ({
  ...
  const baseStyles =
    "px-6 py-2 rounded-full text-sm font-medium relative cursor-pointer hover:scale-105 transition-all duration-200 inline-block text-center";
